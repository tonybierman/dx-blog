...
pub async fn variant_urls_for_media_db(
    ...
    sqlx::query_scalar("SELECT url FROM media_variants WHERE media_id = ?")
        .bind(media_id)
        .fetch_all(pool)
        .await
}
...
pub async fn media_without_variants_db(
    pool: &SqlitePool,
...
pub async fn attach_card_variants(
    pool: &SqlitePool,
    cards: &mut [PostCard],
) -> Result<(), sqlx::Error> {
...
pub async fn media_usage_db(pool: &SqlitePool, url: &str) -> Result<Vec<PostUsage>, sqlx::Error> {
